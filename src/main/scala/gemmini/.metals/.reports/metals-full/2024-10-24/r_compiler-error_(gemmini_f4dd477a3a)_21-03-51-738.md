  ...
  when(cmd.valid && is_loop_cmd && !loop_being_configured.configured) {
    ...
    switch (cmd.bits.cmd.inst.funct) {
      ...
      is (LOOP_WS) {
        ...
        loop_being_configured.label_valid := io.in_label_valid// there are several in ports for assigning the labels to the coarse grained LoopConv/ LoopMatmul, however, @@
      }
    }
  }
