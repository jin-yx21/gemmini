...
action parameters:
offset: 66888
uri: file://<WORKSPACE>/LoopConv.scala
text:
```scala
...
class LoopConv (block_size: Int, coreMaxAddrBits: Int, reservation_station_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
  ...
  val is_config_lbl_cmd = cmd.bits.cmd.inst.funct === CONFIG_CMD && cmd.bits.cmd.rs1(1, 0) === CONFIG_LOAD && cmd.bits.cmd.rs2(32) === 1.U
  ...
  val status_bits = Wire(0.U(1.W))
  for (@@)
  for(i <- 0 until concurrent_loops){
    when(cmd.valid && loop_configured && loops(i).all_completed() && loops(i).label_valid && loops(i).label === label_rawcmd){
...
object LoopConv {
  def apply(in: DecoupledIO[GemminiCmd], ld_completed: UInt, st_completed: UInt, ex_completed: UInt,
    ...
    (mod.io.out, mod.io.busy,mod.io.label,mod.io.label_valid)
  }
