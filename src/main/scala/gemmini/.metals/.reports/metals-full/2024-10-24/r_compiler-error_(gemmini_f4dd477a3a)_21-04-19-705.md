file://<WORKSPACE>/LoopMatmul.scala
...
action parameters:
offset: 36649
uri: file://<WORKSPACE>/LoopMatmul.scala
text:
```scala
  ...
  when(cmd.valid && is_loop_cmd && !loop_being_configured.configured) {
    ...
    switch (cmd.bits.cmd.inst.funct) {
      ...
      is (LOOP_WS) {
        ...
        loop_being_configured.label_valid := io.in_label_valid// there are several in ports for assigning the labels to the coarse grained LoopConv/ LoopMatmul, however, only need to assign the label once, @@
      }
    }
  }
...
object LoopMatmul {
  def apply(label: UInt, label_valid: Bool, in: DecoupledIO[GemminiCmd], ld_completed: UInt, st_completed: UInt, ex_completed: UInt,
    ...
    (mod.io.out, mod.io.busy, mod.io.out_label, mod.io.out_label_valid)
  }
